...
GlobalGraph.G = nx.read_gml('network50.gml') #load the graph
...
for b in range(0,5):
    ...
    for msg in sent:
        ...
        if ms not in ar:
            ...
            if s and d:
                    S,delta = dmc.mincutd(settings.graphlist, int(s),int(d),int(t))
                    J = dmf.maxflow(settings.graphlist, int(s),int(d),int(t))
                    print("There exist : ",len(J)," Disjoint paths")
                    #if len(S) == 0:#This means that source and destination do not have a path.
                    fs = open("Data\\"+str(b)+"Error_MINCUT_d.txt",'a+')
                    fs.write("S* for: \t"+str(s)+"-"+str(d)+"\n")
                    fs.write("d: "+str(delta)+"\n")
                    fs.write(str(S)+"\n")
                    fs.write("----------------------------------\n")
                    
                    ff = open("Data\\"+str(b)+"Error_MAXFLOW_d.txt",'a+')
                    ff.write("J for: \t"+str(s)+"-"+str(d)+"\n")
                    ff.write("There exist : "+str(len(J))+" Disjoint paths:\n")
                    ff.write(str(J)+"\n")
                    ff.write("----------------------------------\n")

                    fn = open("Data\\"+str(b)+"Error_MAXFLOW_d_num.txt",'a+')
                    fn.write(str(len(J))+"\n")
                    
                    fl = open("Data\\"+str(b)+"Error_MINCUT_d_num.txt",'a+')                   
                    fl.write(str(delta)+"\n")
    ...
    for msg in sent:
        ...
        if ms in ar:
            ...
            if s and d:
                    ...
                    print("There exist : ",len(J)," Disjoint paths")
                    #if len(S) == 0:#This means that source and destination do not have a path.
                    fs = open("Data\\"+str(b)+"C_MINCUT_d.txt",'a+')
                    fs.write("S* for: \t"+str(s)+"-"+str(d)+"\n")
                    fs.write("d: "+str(delta)+"\n")
                    fs.write(str(S)+"\n")
                    fs.write("----------------------------------\n")
                    
                    ff = open("Data\\"+str(b)+"C_MAXFLOW_d.txt",'a+')
                    ff.write("J for: \t"+str(s)+"-"+str(d)+"\n")
                    ff.write("There exist : "+str(len(J))+" Disjoint paths:\n")
                    ff.write(str(J)+"\n")
                    ff.write("----------------------------------\n")

                    fn = open("Data\\"+str(b)+"C_MAXFLOW_d_num.txt",'a+')
                    fn.write(str(len(J))+"\n")
                    
                    fl = open("Data\\"+str(b)+"C_MINCUT_d_num.txt",'a+')                   
                    fl.write(str(delta)+"\n")
    ...
    for i in range(0,len(settings.graphlist)):#for every timeslot/time instant of graph
        ...
        for line in c1:
            ...
            sq.append(int(line[6]))
       
        ############################################################
        # Send the packets the same timeslots like the previous model 
        for intex in iter(range(0,len(ts))):
            if ts[intex] == i:
                flag = pp.produce_packet(s[intex],d[intex],"---MSG -> I am \t"+str(s[intex])+"\t and I send to\t"+str(d[intex])+"\t---",sq[intex])
                if flag ==1:
...
##############################################################################
    ...
    for i in range(0,len(settings.graphlist)):#for every timeslot/time instant of graph
        ...
        rlt.reduce_lifetime(2,i) #reduce every time slot the lifetime - 2 --> life of packet is 50 time slots.        #1) Forward -->
        print("------------SEND PROCESS-------------")
        for w in range(0,5): # 5 - Send every active node 
            for n in range(0,len(settings.nodesList)):
                fp.Multicast_Check_Packet(n)
